...
```{bash}
grep "^Cruise" ../data/geotraces/GEOTRACES_IDP2017_v1_CTD_Sensor_Data/GEOTRACES_IDP2017_v1_CTD_Sensor_Data.odv_1c69fb81_1.txt > ../data/geotraces/GEOTRACES_IDP2017_v1_CTD_Sensor_Data/GA10_CTD.tsv

grep "^GA10" ../data/geotraces/GEOTRACES_IDP2017_v1_CTD_Sensor_Data/GEOTRACES_IDP2017_v1_CTD_Sensor_Data.odv_1c69fb81_1.txt >> ../data/geotraces/GEOTRACES_IDP2017_v1_CTD_Sensor_Data/GA10_CTD.tsv
```
...
```{r message=FALSE, warning=FALSE}
GA10.mg <- read_tsv(here::here("data", "geotraces", "geotraces_formatted_metadata.tsv")) %>% 
  filter(section=="GA10") %>% 
...
```{r message=FALSE, warning=FALSE}
GA10.CTD <- read_tsv(here::here("data", "geotraces", "GEOTRACES_IDP2017_v1_CTD_Sensor_Data", "GA10_CTD.tsv"), col_names = TRUE) %>%
  janitor::clean_names() %>%
...
```{r}
GA10.CTD.chla1 <- left_join(GA10.CTD, select(GA10.CTD.dcm_d, -station)) %>%
  group_by(lat, lon, station) %>%
  mutate(chla_moving_average = roll_mean(fluorescence_chl_a_mg_m_3, 10, align="center", fill=NA)) %>%
  fill(chla_moving_average, .direction = "updown") %>%
  mutate(chla_norm = scale(chla_moving_average, center=F)) %>%
...
GA10.CTD.dcm_d1 <- GA10.CTD.chla1 %>%
  filter(depth > 1) %>%
  group_by(lat, lon, station) %>%
...
```{r message=FALSE, warning=FALSE}
ggplot() +
  geom_path(data=GA10.CTD.chla1, aes(y=depth, x=chla_norm, color=chla_norm, group=station), size=0.25) +
  scale_color_viridis_c(option="C") +
  geom_point(data=GA10.CTD.dcm_d1 %>% filter(dcm.layer==1), 
...
ggplot() +
  geom_path(data=GA10.CTD.chla1, aes(y=depth, x=chla_norm, color=chla_norm), size=0.25) +
  scale_color_viridis_c(option="C") +
  geom_point(data=GA10.CTD.dcm_d1 %>% filter(dcm.layer==1), 
             aes(y=depth, x=chla_norm), color="red") +
  facet_wrap(~station) +
...
```{r}
GA10.mg.convert <- geo_left_join(GA10.mg, distinct(GA10.CTD.dcm_d1, lon, lat), 
                        by = c("lat","lon"),
...
```{r}
GA10.dcm.a <- fuzzy_left_join(GA10.mg.convert, GA10.CTD.dcm_d1, 
                      by=c("lon" = "lon",
                           "lat" = "lat",
                        "depthmin" = "depth",
                        "depthmax" = "depth"),
...
GA10.dcm.b <- GA10.mg %>% filter(sampleID %nin% pull(GA10.dcm.a, sampleID)) %>%
  mutate(chla_norm=0, dcm.layer=0, dcm.max=0) %>%
  select(sampleID, chla_norm, dcm.layer, dcm.max) 

GA10.dcm.final <- bind_rows(GA10.dcm.a, GA10.dcm.b) %>%
  left_join(select(GA10.mg, -depthmin, -depthmax), .)  %>%
  group_by(lat, lon) %>%
...
```{r}
write_tsv(GA10.dcm.final, here::here("data", "intermediate_files", "GA10_mg_ctd.tsv"))
```
