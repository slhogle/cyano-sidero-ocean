...
```{bash}
grep "^Cruise" ../data/geotraces/GEOTRACES_IDP2017_v1_CTD_Sensor_Data/GEOTRACES_IDP2017_v1_CTD_Sensor_Data.odv_1c69fb81_1.txt > ../data/geotraces/GEOTRACES_IDP2017_v1_CTD_Sensor_Data/GA03_CTD.tsv

grep "^GA03" ../data/geotraces/GEOTRACES_IDP2017_v1_CTD_Sensor_Data/GEOTRACES_IDP2017_v1_CTD_Sensor_Data.odv_1c69fb81_1.txt >> ../data/geotraces/GEOTRACES_IDP2017_v1_CTD_Sensor_Data/GA03_CTD.tsv
```
...
```{r message=FALSE, warning=FALSE}
GA03.mg <- read_tsv(here::here("data", "geotraces", "geotraces_formatted_metadata.tsv")) %>% 
  filter(section=="GA03") %>% 
...
```{r message=FALSE, warning=FALSE}
GA03.CTD <- read_tsv(here::here("data", "geotraces", "GEOTRACES_IDP2017_v1_CTD_Sensor_Data", "GA03_CTD.tsv"), col_names = TRUE) %>%
  janitor::clean_names() %>%
...
```{r}
GA03.CTD.chla1 <- left_join(GA03.CTD, select(GA03.CTD.dcm_d, -station)) %>%
  group_by(lat, lon, station) %>%
  mutate(chla_moving_average = roll_mean(fluorescence_raw_data_arbitrary_units, 10, align="center", fill=NA)) %>%
  fill(chla_moving_average, .direction = "updown") %>%
  mutate(chla_norm = scale(chla_moving_average, center=F)) %>%
...
GA03.CTD.dcm_d1 <- GA03.CTD.chla1 %>%
  filter(depth > 1) %>%
  group_by(lat, lon, station) %>%
...
```{r message=FALSE, warning=FALSE}
ggplot() +
  geom_path(data=GA03.CTD.chla1, aes(y=depth, x=chla_norm, color=chla_norm, group=station), size=0.25) +
  scale_color_viridis_c(option="C") +
  geom_point(data=GA03.CTD.dcm_d1 %>% filter(dcm.layer==1), 
...
ggplot() +
  geom_path(data=GA03.CTD.chla1, aes(y=depth, x=chla_norm, color=chla_norm), size=0.25) +
  scale_color_viridis_c(option="C") +
  geom_point(data=GA03.CTD.dcm_d1 %>% filter(dcm.layer==1), 
             aes(y=depth, x=chla_norm), color="red") +
  facet_wrap(~station) +
...
```{r}
GA03.mg.convert <- geo_left_join(GA03.mg, distinct(GA03.CTD.dcm_d1, lon, lat), 
                        by = c("lat","lon"),
...
```{r}
GA03.dcm.a <- fuzzy_left_join(GA03.mg.convert, GA03.CTD.dcm_d1, 
                      by=c("lon" = "lon",
                           "lat" = "lat",
                        "depthmin" = "depth",
                        "depthmax" = "depth"),
...
GA03.dcm.b <- GA03.mg %>% filter(sampleID %nin% pull(GA03.dcm.a, sampleID)) %>%
  mutate(chla_norm=0, dcm.layer=0, dcm.max=0) %>%
  select(sampleID, chla_norm, dcm.layer, dcm.max) 

GA03.dcm.final <- bind_rows(GA03.dcm.a, GA03.dcm.b) %>%
  left_join(select(GA03.mg, -depthmin, -depthmax), .)  %>%
  group_by(lat, lon) %>%
...
```{r}
write_tsv(GA03.dcm.final, here::here("data", "intermediate_files", "GA03_mg_ctd.tsv"))
```
